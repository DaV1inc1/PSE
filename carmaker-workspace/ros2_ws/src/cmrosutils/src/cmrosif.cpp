...
int CarMakerROSInterface::init(struct tInfos* infofile_ptr) {
  ...
  if (use_sim_time_) {
    ...
    job->setCallbackHook(CMJob::CallbackHook::StartCycle);
...
int CarMakerROSInterface::testrunStartAtBegin(struct tInfos* infofile_ptr) {
  ...
  if (!SimCore.IsRegularInit) {
    Log::printLog("Disabling CarMaker ROS Node during first init...");
  }

  if (!infofile_ptr) {
    Log::printLog("Missing Infofile!");
  }
  ...
  {
    std::stringstream ss;
    ss << "CarMaker ROS Node enabled with ";
    ss << (sync_mode_ == SyncMode::kTopic ? "topic-based" : "disabled");
    ss << " synchronization";
...
void CarMakerROSInterface::in() {
  ...
  scheduler_.startCycle();

  if (sync_mode_ == SyncMode::kTopic) {
    scheduler_.lock([&]() { rclcpp::spin_some(nhp_); });
  } else {
    rclcpp::spin_some(nhp_);
  }
  ...
  scheduler_.executeJobs(CMJob::CallbackHook::In);
}
...
int CarMakerROSInterface::drivmanCalc(const double& dt) {
  ...
  scheduler_.executeJobs(CMJob::CallbackHook::DrivMan);
...
int CarMakerROSInterface::vehicleControlCalc(const double& dt) {
  ...
  scheduler_.executeJobs(CMJob::CallbackHook::VehicleControl);
...
int CarMakerROSInterface::calc(const double& dt) {
  ...
  scheduler_.executeJobs(CMJob::CallbackHook::Calc);
...
void CarMakerROSInterface::out() {
  ...
  scheduler_.executeJobs(CMJob::CallbackHook::Out);
  scheduler_.endCycle();
}
